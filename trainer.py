...
# MODEL_CLASSES = {
#     ## bert
#     'bert': (BertConfig, BertJointNlu, AutoTokenizer),
...
def train(args, train_dataset, model, tokenizer, ema=None):
    ...
    linear_param_optimizer = list(model.intent_classifier.named_parameters()) + list(model.slot_decoder.named_parameters())
    ...
    best_f1, best_intent_f1, best_slot_f1 = 0.0, 0.0, 0.0
    no_gain_counter = 0
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {
                ...
                "token_type_ids": batch[2] if args.model_type in ["bert_nlu"] else None
            }
            if args.speaker_segment:
                inputs["token_type_ids"] = batch[6]
            nlu_loss, nlu_logits = model(**inputs)
            loss, slot_loss, intent_loss = nlu_loss.total_loss, nlu_loss.slot_loss, nlu_loss.intent_loss
            if args.n_gpu > 1:
                loss = loss.mean()
            ...
            if args.gradient_accumulation_steps > 1:
                ...
                slot_loss = slot_loss / args.gradient_accumulation_steps
            ...
            pbar(step, {'loss': loss.item(), "intent loss": intent_loss.item(), "slot loss": slot_loss.item()})
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and global_step % 50 == 0:
                    ...
                    kl_loss_intent, kl_loss_slot = nlu_loss.kl_loss_intent, nlu_loss.kl_loss_slot
                    wandb.log({
                        "train/total_loss": loss.item(),
                        "train/intent_loss": intent_loss.item(),
                        "train/slot_loss": slot_loss.item(),
                        "train/kl_loss_intent": kl_loss_intent,
                        "train/kl_loss_slot": kl_loss_slot,
                ...
                if args.local_rank in [-1, 0] and args.evaluate_during_training and global_step % args.logging_steps == 0:
                    ...
                    slot_f1 = float(eval_results["slot_micro_f1"])
                    mean_f1 = (intent_f1 + slot_f1) * 0.5
                    
                    if mean_f1 > best_f1 or (intent_f1 > best_intent_f1 and slot_f1 > best_slot_f1):
                        logger.info(f"best f1 now update: ")
                        logger.info(f"intent f1: {best_intent_f1} -> {intent_f1}, slot f1: {best_slot_f1} -> {slot_f1}, best total f1: {best_f1} -> {mean_f1}")
                    ...
                    best_slot_f1 = max(slot_f1, best_slot_f1)
                    best_f1 = max(mean_f1, best_f1)

                    wandb.log({
                        "eval/slot_micro_f1": slot_f1,
                        "eval/intent_micro_f1": intent_f1,
                        "eval/eval_loss": float(eval_results["loss"]),
                    }, step=global_step)

                    # 意图、槽位都没有进一步变好, 计数器加1
                    if intent_f1 < best_f1 and slot_f1 < best_slot_f1:
                        no_gain_counter += 1
...
@torch.no_grad()
def evaluate(args, model, tokenizer, prefix="", step=None, ema=None):
    if ema is not None:
        ema.apply_shadow()
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        slot_logits = nlu_logits.slot_logits
        # TODO: 增加参数区分多分类和多标签
        ...
        tags = model.slot_decoder.decode(slot_logits, slot_mask)
    ...
    logger.info(f"Slot Report for Step {prefix}: ")
    ...
    logger.info("")
    logger.info(f"Intent Report for Step {prefix}: ")
    ...
    results = {"loss": eval_loss, "slot_micro_f1": slot_micro_f1, "intent_micro_f1": inent_micro_f1}
    for idx, sentence in enumerate(sentence_list):
        sentence = sentence.split("[SEP]")
        context, query = sentence[0], sentence[1]
        line = f"{context}\t{query}\t{logging_intent_labels[idx]}\t{logging_intent_preds[idx]}\t{logging_label_entities[idx]}\t{logging_pred_entities[idx]}\t{slot_true_list[idx]}\t{slot_pred_list[idx]}"
    ...
    with open(csv_output_path, "w") as fp:
        fp.write("context\tquery\tintent_label\tintent_pred\ttrue_entities\tpred_entities\ttrue_markup\tpred_markup\n")
        fp.write("\n".join(logging_data))
    if ema is not None:
        ema.restore()
    return results
...
def export(args, model, tokenizer, type="trace"):
    ...
    if type == "trace":
        ...
        lego_model2 = torch.jit.load(os.path.join(saved_dir, "lego_model_v2", "model.pt"))
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            if args.model_type != "distilbert":
                ...
                inputs["token_type_ids"] = (batch[2] if args.model_type in ["bert_nlu"] else None)
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    processor = processors[task](task_name=task, slot_markup=args.markup)
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        intent_label_list, slot_label_list = processor.get_labels(slot_markup=args.markup)
        if data_type == 'train':
...
def main():
    ...
    processor = processors[args.task_name](task_name=args.task_name)
    intent_label_list, slot_label_list = processor.get_labels(slot_markup=args.markup)

    args.intent_id2label = {i: label for i, label in enumerate(intent_label_list)}
    args.slot_id2label = {i: label for i, label in enumerate(slot_label_list)}
    args.intent_label2id = {label: i for i, label in enumerate(intent_label_list)}
    args.slot_label2id = {label: i for i, label in enumerate(slot_label_list)}
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if prefix == "":
                continue
            model = model_class.from_pretrained(checkpoint, config=config, args=args, num_intent_labels=len(intent_label_list), num_slot_labels=len(slot_label_list))
            model.to(args.device)
            result = evaluate(args, model, tokenizer, prefix=prefix, ema=ema)
            logger.info(result["slot_micro_f1"])
            logger.info("\n")
    ...
    if args.do_export and args.local_rank in [-1, 0]:
        ...
        global_step = checkpoint.split("-")[-1]
        model = model_class.from_pretrained(checkpoint, config=config, args=args, num_intent_labels=len(intent_label_list), num_slot_labels=len(slot_label_list), do_export=True)
        export(args, model, tokenizer, type="s")
