...
def train(args, train_dataset, model, tokenizer, ema=None):
    ...
    linear_param_optimizer = list(model.slot_decoder.named_parameters())
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {
                ...
                "token_type_ids": batch[2] if args.model_type in ["bert_slot"] else None
            }
            if args.speaker_segment:
                inputs["token_type_ids"] = batch[6]
            nlu_loss, nlu_logits = model(**inputs)
            loss, slot_loss, kl_loss = nlu_loss.total_loss, nlu_loss.slot_loss, nlu_loss.kl_loss_slot
            if args.n_gpu > 1:
                loss = loss.mean()
            ...
            pbar(step, {'loss': loss.item(), "slot loss": slot_loss.item(), "kl loss": kl_loss.item()})
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.evaluate_during_training and global_step % args.logging_steps == 0:
                    ...
                    if slot_f1 > best_f1:
                        ...
                        logger.info(f"slot f1: {best_f1} -> {slot_f1}")
...
def evaluate(args, model, tokenizer, prefix="", step=None, ema=None):
    ...
    results = {"loss": eval_loss, "slot_micro_f1": slot_micro_f1}
    for idx, sentence in enumerate(sentence_list):
        sentence = sentence.split("[SEP]")
        context, query = sentence[0], sentence[1]
        line = f"{context}\t{query}\t{logging_label_entities[idx]}\t{logging_pred_entities[idx]}\t{slot_true_list[idx]}\t{slot_pred_list[idx]}"
    ...
    with open(csv_output_path, "w") as fp:
        fp.write("context\tquery\ttrue_entities\tpred_entities\ttrue_markup\tpred_markup\n")
        fp.write("\n".join(logging_data))
    if ema is not None:
        ema.restore()
    return results
...
def export(args, model, tokenizer, type="trace"):
    ...
    if type == "trace":
        ...
        logits = jit_model(inputs['input_ids'].to("cuda"),inputs['attention_mask'].to("cuda"),inputs['token_type_ids'].to("cuda"))
        ...
        sentence_list, logging_data = [], []
        slot_metric = SlotMetrics(args.slot_id2label)
        lego_model = torch.jit.load(os.path.join(saved_dir, "lego_model", "model.pt"))
        lego_model.eval()
        for step, batch in enumerate(eval_dataloader):
            batch = tuple(t.to(args.device) for t in batch)
            inputs = {
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            if args.model_type != "distilbert":
                ...
                inputs["token_type_ids"] = (batch[2] if args.model_type in ["bert_slot"] else None)
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if prefix == "":
                continue
            model = model_class.from_pretrained(checkpoint, config=config, args=args, num_slot_labels=len(slot_label_list))
            model.to(args.device)
    ...
    if args.do_export and args.local_rank in [-1, 0]:
        ...
        global_step = checkpoint.split("-")[-1]
        model = model_class.from_pretrained(checkpoint, config=config, args=args, num_slot_labels=len(slot_label_list), do_export=True)
        export(args, model, tokenizer, type="y")
